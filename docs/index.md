# 可积系统研究简报

面向可积系统、反散射、黎曼--希尔伯特方法、非线性波和相关课题组资料的研究向入口。

[最新简报](radar/latest.md) · [简报归档](radar/index.md) · [资源](resources.md) · [核心主题](topics.md)

!!! note "AI 生成说明"
    日期为本站整理日期。条目由 AI 辅助检索和整理，主要依据题名、摘要、分类、关键词和公开元数据筛选；推荐表示相关性和阅读优先级，不代表论文正确性已经核验。数学结论请以原论文为准。

## 2026-07-09

### 近期关注

- **Asymptotic analysis of N-elliptic localized solutions for the Fokas--Lenells equation**  
  Wang Tang, Guo-Fu Yu · [arXiv:2607.06409](https://arxiv.org/abs/2607.06409) · `Fokas--Lenells` `Darboux--Bäcklund` `elliptic background`

- **Integrable full discretization of the multi-component short pulse equation**  
  Ayako Hori, Yuta Tanaka, Ken-ichi Maruno, Yasuhiro Ohta, Bao-Feng Feng · [arXiv:2607.04756](https://arxiv.org/abs/2607.04756) · `multi-component` `short pulse` `integrable discretization`

- **Sharp Upper Bound for Amplitudes of Finite-Gap Solutions of the Modified Korteweg-de Vries Equation**  
  Otis C. Wright · [arXiv:2607.01348](https://arxiv.org/abs/2607.01348) · `finite-gap` `mKdV` `amplitude bounds`

### 值得补读

- **Asymptotics of Soliton Gas for the Derivative Nonlinear Schrödinger Equation**  
  Deng Shan Wang, Xinyu Wang · [arXiv:2606.24435](https://arxiv.org/abs/2606.24435) · `DNLS` `soliton gas` `RHP` `∂̄`

- **Inverse scattering for the linear system associated with the coupled Gerdjikov--Ivanov equations**  
  Ramazan Ercan · [arXiv:2605.28856](https://arxiv.org/abs/2605.28856) · `coupled GI` `Jost solutions` `Marchenko equations`

- **Orbital stability of breathers for coupled nonlinear Schrödinger equations**  
  Liming Ling, Dmitry E. Pelinovsky, Huajie Su · [Advances in Mathematics 495 (2026), 110961](https://dmpeli.math.mcmaster.ca/PaperBank/BreathersCoupledNLS.pdf) · `coupled NLS` `breathers` `orbital stability`

[查看完整简报](radar/latest.md)

## 其他入口

- [Resources / 资源](resources.md)：课程、讲义、研究者主页和检索入口。
- [Group work / 课题组相关](group-work.md)：本地研究背景、公开笔记和相关链接。
- [Core topics / 核心主题](topics.md)：本站目前关注的方程、方法和非线性波主题。
- [About / 关于](about.md)：策展原则、设计参考、AI 使用和许可证说明。
